...
export default function BlogsPagination({ sectionsData }: {sectionsData: SectionCategory[]}) {
  ...
  const fetchBlogs = async (): Promise<ArticleListResponse> => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/getAllArticles?page=${currentPage}&term=${inputValue}&blog_section_id=${selectedSectionId}`, {
      method: "GET",
      headers: {
    ...
    <>
        <div className="flex flex-col-reverse lg:flex-row justify-between items-start lg:items-center gap-6 mb-[32px] md:border-b-2 md:border-[#F0ECE7] md:pb-[22px]">
            <div className="hidden md:flex flex-wrap gap-[32px] lg:gap-y-[40px] xl:gap-6 px-[15px] xl:px-0">
                <div onClick={() => setSelectedSectionId("")} className={`${selectedSectionId == "" ? "blog-icon-container": ""} flex flex-col items-center gap-2 relative cursor-pointer`}>
                    <svg
            ...
            {sectionsData.map(section => {
                return (
                      <div onClick={() => setSelectedSectionId(String(section.id))} key={section.id} className={`${String(section.id) == selectedSectionId ? "blog-icon-container": ""} flex flex-col cursor-pointer items-center gap-2 relative`}>
                          <svg
                              className="blogs-icon"
                              xmlns="http://www.w3.org/2000/svg"
                              width="69"
            ...
            <section className="block md:hidden w-full px-[10px] xl:px-0 pb-[0px] border-b-2 border-[#F0ECE7]">
                <BlogsCategories sectionsData={sectionsData} setSelectedSection={value => setSelectedSectionId(value)} selectedSectionId={selectedSectionId} />
            </section>
            ...
            <div className="w-full lg:w-[439px] px-[15px] xl:px-0">
                ...
                <div className="flex items-center justify-between bg-white border border-[#DADADA] rounded-lg pe-[8px] py-3 h-14">
                    <form className="flex items-center gap-3 flex-1 ps-[16px]" onSubmit={(e) => {
                        e.preventDefault()
                        refetch()
                    }}>
    ...
    page === "..." ? (
      ...
      <button
        ...
        className={`border border-[#131A27] rounded-[8px] h-[44px] w-[44px] flex justify-center items-center font-bold transition ${
          currentPage === page
            ? "bg-[#131A27] text-white"
            : "hover:bg-[#131A27] hover:text-white"
        }`}
      >
        {page}
      </button>
