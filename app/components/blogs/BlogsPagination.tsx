...
export default function BlogsPagination({ sectionsData }: { sectionsData: SectionCategory[] }) {
  ...
  const fetchBlogs = async (): Promise<ArticleListResponse> => {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/getAllArticles?page=${currentPage}&term=${inputValue}&blog_section_id=${selectedSectionId}`,
      {
        method: "GET",
        headers: {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["blogs", locale, currentPage, selectedSectionId],
    queryFn: fetchBlogs,
  });
